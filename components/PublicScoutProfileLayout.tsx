...
import { ArrowLeft, Heart, Link as LinkIcon, MapPin, MessageCircle, Share2, Tag, UserPlus, UserCheck } from 'lucide-react-native';
...
const SCOUT_CONNECTION_SHEET_TABS: ProfileConnectionsSheetTab[] = [
  { key: 'followers', title: 'Followers' },
  { key: 'fans', title: 'Fans' },
  { key: 'following', title: 'Following' },
];

type ScoutTab = 'liked' | 'tagged';

type PublicScoutProfileLayoutProps = {
  profile: User;
  onBackPress: () => void;
...
export function PublicScoutProfileLayout({
  profile,
  onBackPress,
  isTabFocused,
  orgName,
  ...
  onFollowPress,
  onMessagePress,
}: PublicScoutProfileLayoutProps) {
  const bgStyle = useThemeBackgroundStyle();
  const { posts } = useFeedPosts();
  const [activeTab, setActiveTab] = useState<ScoutTab>('liked');
  ...
  return (
    <View style={[styles.container, bgStyle]}>
        ...
        <View style={styles.contentContainer}>
          ...
          <View style={styles.stats}>
            ...
            <View style={styles.stat}>
              ...
              <Text style={styles.label}>SCOUTED</Text>
            </View>
          </View>
          ...
          <View style={styles.tabs}>
            ...
            <TouchableOpacity style={styles.tab} onPress={() => setActiveTab('tagged')}><Tag size={20} color={activeTab === 'tagged' ? Colors.primary : Colors.textSecondary} /></TouchableOpacity>
          </View>

          {activeTab === 'liked' ? profilePosts.map((post) => <FeedCard key={post.id} post={post} isVisible={isTabFocused} />) : <Text style={styles.empty}>No tagged content</Text>}
          {activeTab === 'liked' && profilePosts.length === 0 ? <Text style={styles.empty}>No public activity yet</Text> : null}
        </View>
      </ScrollView>
      ...
      <ProfileConnectionsSheet
        ...
        tabs={SCOUT_CONNECTION_SHEET_TABS}
      />
    </View>
  );
}

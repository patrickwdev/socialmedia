...
import { MOCK_FOLLOWERS, MOCK_FANS, MOCK_FOLLOWING, MOCK_SUGGESTED, MOCK_CONNECT_PEOPLE, type FollowerItem, type Post, type User } from '@/data/mock';
import { PostMedia } from '@/components/PostMedia';
import { ArrowLeft, Share2, MoreHorizontal, Pencil, BadgeCheck, Grid, Film, Repeat, Tag, UserPlus, UserCheck, Search, MapPin, User as UserIcon, MessageCircle, Send, Instagram, Twitter, Copy, BarChart3, Dumbbell, Star } from 'lucide-react-native';
...
export type AthleteProfileLayoutProps = {
  profile: User;
  variant: 'self' | 'public';
  ...
  onBackPress?: () => void;
  /** Public profile: live follow state from parent (Supabase `profile_follows`). */
  isFollowing?: boolean;
  followBusy?: boolean;
  onFollowPress?: () => void;
  /** Public profile: fan state/action (separate from follow). */
  isFan?: boolean;
  fanBusy?: boolean;
  onFanPress?: () => void;
  /** Public profile: open DM / messages for this user. */
  onMessagePress?: () => void;
};

export function AthleteProfileLayout({
  profile,
  variant,
  sportSubtitle,
  profileLink,
  ...
  const handleShareProfile = async () => {
    ...
    const message =
      variant === 'self'
        ? `Check out my profile: ${shareProfileUrl}`
        : `Check out @${profile.username}: ${shareProfileUrl}`;
    try {
      await Share.share({
        message,
        url: shareProfileUrl,
        ...
        <View style={styles.coverContainer}>
            ...
            <SafeAreaView style={styles.headerActions}>
                <TouchableOpacity
                  ...
                  onPress={variant === 'public' ? onBackPress : undefined}
                  accessibilityRole={variant === 'public' ? 'button' : undefined}
                  accessibilityLabel={variant === 'public' ? 'Go back' : undefined}
                >
        ...
        <View style={styles.contentContainer}>
            ...
            <View style={styles.profileHeader}>
                ...
                <View style={styles.actionButtons}>
                    <TouchableOpacity style={styles.followButton} onPress={openSharePanel} accessibilityRole="button" accessibilityLabel="Share profile">
                    ...
                    {variant === 'self' ? (
                      ...
                      <>
                        <TouchableOpacity
                          style={styles.followButton}
                          onPress={onFollowPress ?? (() => Alert.alert('Follow', 'Coming soon.'))}
                          disabled={followBusy}
                          accessibilityRole="button"
                        ...
                        <TouchableOpacity
                          style={styles.followButton}
                          onPress={onFanPress ?? (() => Alert.alert('Fan', 'Coming soon.'))}
                          disabled={fanBusy}
                          accessibilityRole="button"
                          accessibilityLabel={isFan ? 'Unfan' : 'Fan'}
                        >
                          <Star
                            size={20}
                            color={isFan ? Colors.primary : Colors.text}
                            fill={isFan ? Colors.primary : 'transparent'}
                          />
                        </TouchableOpacity>
                        <TouchableOpacity
                          style={styles.followButton}
                          onPress={onMessagePress ?? (() => Alert.alert('Message', 'Coming soon.'))}
                          accessibilityRole="button"
